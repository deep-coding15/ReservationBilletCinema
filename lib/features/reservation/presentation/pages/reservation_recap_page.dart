...
/// Page récap réservation CINÉMA : un type + options par place (par billet).
class ReservationRecapPage extends StatefulWidget {
  const ReservationRecapPage({super.key, this.recapData});

  final ReservationRecapData? recapData;

  @override
  State<ReservationRecapPage> createState() => _ReservationRecapPageState();
}

class _ReservationRecapPageState extends State<ReservationRecapPage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (data == null || data.siegeIds.isEmpty) {
      return Scaffold(
        ...
        body: const Center(
          child: Text('Données de réservation manquantes.', style: TextStyle(color: Colors.white70)),
        ),
      );
    }

    if (_lines.length != data.siegeIds.length) {
      _lines = List.generate(data.siegeIds.length, (_) => const TicketLine());
    }

    final total = ReservationRecapData(seance: data.seance, siegeIds: data.siegeIds, lines: _lines).total;
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      '${data.seance.cinemaNom ?? 'Cinéma'} — ${data.siegeIds.length} place(s)',
                      style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600),
                    ),
                    const SizedBox(height: 4),
                    Text(
                      'Prix de base: ${data.seance.prix.toStringAsFixed(0)} DH / place',
            ...
            SizedBox(
              ...
              child: FilledButton(
                onPressed: () {
                  ...
                  context.push('/reservation/paiement', extra: payload);
                },
          ...
          children: [
            Row(
              children: [
                Text(
                  'Billet $billetNum',
                  style: const TextStyle(color: AppColors.neon, fontSize: 16, fontWeight: FontWeight.w600),
                ),
                const Spacer(),
                Text(
                  '${lineTotal.toStringAsFixed(0)} DH',
            ...
            Wrap(
              ...
              children: TicketLine.typeNames.map((t) {
                ...
                return ChoiceChip(
                  ...
                  selected: selected,
                  onSelected: (_) {
                    // VIP : options incluses automatiquement ; Normal : options vides
